...
/**
 * The executability gate for beam-rank's declared Data classes (api-surface-coherence ticket 85).
 *
 * Testbench does not auto-discover, so a package harness boots exactly what `getPackageProviders()`
 * names while `src/` freely imports anything it can autoload. Measured here before the fix:
 * `tests/TestCase.php` never registered `Spatie\LaravelData\LaravelDataServiceProvider`, so
 * `config('data')` was NULL and `RankTreeInputData::validateAndCreate()` died with
 ...
 * TypeScript, the OpenAPI document and the client SDK all agree with a DTO that would throw in
 * production otherwise. So this walks every declared Data class and executes the two halves of the
 ...
 *
 * Neither needs a fixture payload, which is what lets it be data-driven across every class; a
 * per-class payload test is a finer instrument and belongs with the class that needs one.
 */
function beamRankDataClasses(): array
{
    ...
    foreach ($files as $file) {
        ...
        // Namespace is the path under src/, PSR-4'd onto Splicewire\Beam\Rank\.
        $relative = substr($path, strlen($root) + 1, -4);
        $class = 'Splicewire\\Beam\\Rank\\'.str_replace('/', '\\', $relative);
...
it('finds beam-rank Data classes to check', function () {
    // A discovery bug that silently found zero classes would make every case below vacuous.
    expect(count(beamRankDataClasses()))->toBeGreaterThanOrEqual(5);
});
